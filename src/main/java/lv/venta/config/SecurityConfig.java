...
public class SecurityConfig {
	...
	public UserDetailsManager createTestUsers() {
		...
		UserDetails userD1 = User.builder().username("valdis").password(encoder.encode("")).authorities("USER").build();
	...
	public SecurityFilterChain configureEndpoints(HttpSecurity http) {
		http.authorizeHttpRequests(
				...
				//Pievienot atlikuso kontrolieru endpointus
		);
		http.formLogin(auth->auth.permitAll());//Login forma
